...
$('.calendar_date_select').supercal({
                transition: 'carousel-vertical',
                todayButton: false,
                format: 'm/d/Y',
                date: new Date(document.querySelector("meta[name=selected_date]").content),
            }).on('dateselect', function(e) {
                var date = $(this).data('date').toISOString().slice(0,10);
    ...
    $('tr.clip-row').click(function () {
        $("#myvid > source").attr("src", this.dataset.filename);
        vid.load();
        vid[0].play();
    });
